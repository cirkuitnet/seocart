<?php
/**
 * ThingHappened: a fixture event delivered through the outbox
 *
 * @package SEOCart
 * @since   0.1.0
...
/**
 * An event that must not be lost, shaped like a real one: an id, a summary and a list of ids.
 *
 * @since 0.1.0
 */
final readonly class ThingHappened implements DomainEvent {
	 ...
	 * @param string                  $note       Optional. A summary. Default 'noted'.
	 * @param int[]                   $lineIds    Optional. Ids of related rows. Default 1, 2, 3.
	 * @param \DateTimeImmutable|null $occurredAt Optional. When it happened. Default a fixed instant.
	 */
	public function __construct( int $thingId, string $note = 'noted', array $lineIds = array( 1, 2, 3 ), ?\DateTimeImmutable $occurredAt = null ) {
		$this->thingId    = $thingId;
		$this->note       = $note;
		$this->lineIds    = $lineIds;
		$this->occurredAt = $occurredAt ?? new \DateTimeImmutable( '2026-09-23 10:00:00.123456', new \DateTimeZone( 'UTC' ) );
	}
	...
	public static function fromPayload( array $payload, \DateTimeImmutable $occurredAt, int $version ): self {
		return new self( (int) $payload['thing_id'], (string) $payload['note'], array_map( 'intval', (array) $payload['line_ids'] ), $occurredAt );
	}
