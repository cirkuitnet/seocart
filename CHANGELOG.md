...
- A cart through the Store API: `GET /seocart/store/v1/cart`, `POST /seocart/store/v1/cart/lines` (a batch of lines) and `PATCH /seocart/store/v1/cart/lines/{line_identity}` (a new quantity; 0 removes the line). No cart exists until its first line, and that write is the one that sends the cart token. Every answer carries the cart's version, its lines and its totals from the calculation pipeline; every write must name the version it was based on, and a write based on an older one is refused with `cart.version_stale`, whose details carry the current version and totals. Identical lines merge; a cart holds up to 50 lines and a line up to 99,999 units; a client may start up to 50 carts a day; an expired cart is invisible at once and removed by an hourly job.
