 ...
 * - PermissionFactory's check, and OperationInvoker for the execution.
 *
			...
			wp_register_ability(
				...
				array(
					...
					'execute_callback'    => function ( $input = null ) use ( $operation ): array|WP_Error {
						return $this->invoker->invoke( $operation, is_array( $input ) ? $input : array() );
					},
					'permission_callback' => static function ( $input = null ) use ( $definition ): bool {
						return PermissionFactory::allows( $definition, is_array( $input ) ? $input : array() );
					},
					'meta'                => array(
