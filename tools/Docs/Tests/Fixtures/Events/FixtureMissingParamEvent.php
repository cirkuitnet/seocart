<?php
/**
 * FixtureMissingParamEvent: a fixture event whose constructor is missing an @param sentence
 *
 * @package SEOCart
 * @since   0.1.0
...
/**
 * A fixture event, deliberately missing the @param sentence for one payload property.
 *
 * Fixture only: proves HooksReference refuses to document a payload property it cannot describe,
 * rather than silently leaving it blank, and names the class and the property.
 *
 * @since 0.1.0
 */
final readonly class FixtureMissingParamEvent implements DomainEvent {

	// phpcs:disable Squiz.Commenting.FunctionComment.MissingParamComment -- $quantity's tag is deliberately left without a description; that is the point of this fixture.
	/**
	 * Records the event. $quantity's tag is deliberately left without a description.
	 *
	 * @since 0.1.0
	 *
	 ...
	 * @since 0.1.0
	 *
	 * @return string `fixture_missing_param`.
	 */
	public static function eventName(): string {
		return 'fixture_missing_param';
	}
	...
	public static function fromPayload( array $payload, \DateTimeImmutable $occurredAt, int $version ): self {
		...
		return new self( (int) $payload['bin_id'], (int) $payload['quantity'], $occurredAt );
	}
